...
var definitions = map[Opcode]*Definition{
	...
	// OpCall has an operand that holds the number
...
// Make converts opcode to bytecode
func Make(op Opcode, operands ...int) []byte {
	...
	// confirms what the instruction length will be
	// to allocate the right byte slice
	instructionLen := 1
	...
	offset := 1
	// Iterating over the defined operands to find the match
	for i, o := range operands {
		...
		// in order to handle other opcodes
		offset += width
	}
	return instruction
}
...
// Returns the desired format - easier to read
