...
func TestIntegerArithmetic(t *testing.T) {
	// the first two just tests to see that at a single literal
	// results in a single number being pushed onto the stack
	tests := []vmTestCase{
		{"1", 1},
		{"2", 2},
		{"1 + 2", 3},
...
func testBooleanObject(expected bool, actual object.Object) error {
	...
	if !ok {
		return fmt.Errorf("object is not Boolean. got=%T (%+v)", result.Value, expected)
	}
