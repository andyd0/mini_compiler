	...
	tests := []compilerTestCase{
		{
			...
			expectedInstructions: []code.Instructions{
				...
				// The compiled expression should be followed by an OpPop
...
func TestFunctions(t *testing.T) {
	// expectedInstructions changed from OpConstant
	// to OpClosure to test change to treating all functions
	// as closures
	tests := []compilerTestCase{
		{
	...
	tests := []compilerTestCase{
		{
			...
			expectedConstants: []interface{}{
				[]code.Instructions{
					// a loaded as a free variable for the innermost
					// function
				...
				[]code.Instructions{
					// the outer function and loading a
					// onto the stack which will be used
		...
		},
		{
			...
			expectedConstants: []interface{}{
				...
				[]code.Instructions{
					// OpGetFree also used here because a is
					// also a free variable to the middle function
					code.Make(code.OpGetFree, 0),
					code.Make(code.OpGetLocal, 0),
...
// Take Monkey code as input, we parse it, produce an AST, hand
// it to the compiler and then make assertions about the bytecode
...
// expectedInstructions is a slice of slices of bytes
...
// checks the expected constantsagainst the actual constants the compiler
// produced
func testConstants(
	t *testing.T,
	expected []interface{},
	...
	for i, constant := range expected {
		...
		case string:
			...
			if err != nil {
				return fmt.Errorf("constant %d - testStringObject failedd: %s", i, err)
			}
